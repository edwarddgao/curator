import asyncio
import aiohttp
import sqlite3
import json
import csv
import time
import argparse
import os
import io

MET_API = "https://collectionapi.metmuseum.org/public/collection/v1"
MET_CSV_URL = "https://media.githubusercontent.com/media/metmuseum/openaccess/master/MetObjects.csv"
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(SCRIPT_DIR, "../data/raw_artworks.db")

BURST_SIZE = 80  # requests per burst (API limit, no proxy)
BURST_COOLDOWN = 60  # seconds between bursts (no proxy)
PROXY_CONCURRENCY = 150  # concurrent requests when using proxy
COMMIT_EVERY = 500
MAX_RETRIES = 3


def log(msg):
    print(msg, flush=True)


def init_db(db_path):
    os.makedirs(os.path.dirname(db_path), exist_ok=True)
    conn = sqlite3.connect(db_path)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS raw_artworks (
            met_object_id INTEGER PRIMARY KEY,
            data TEXT,
            is_public_domain BOOLEAN,
            has_image BOOLEAN,
            fetched_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )
    """)
    conn.commit()
    return conn


def get_existing_ids(conn):
    cursor = conn.execute("SELECT met_object_id FROM raw_artworks")
    return {row[0] for row in cursor.fetchall()}


async def get_public_domain_ids_from_csv(session):
    """Download Met CSV and extract public domain object IDs.
    This is much more reliable than the search API pre-filter which has ~28% false positives."""
    log("Downloading Met Museum CSV from GitHub (this may take a minute)...")
    async with session.get(MET_CSV_URL) as resp:
        resp.raise_for_status()
        text = await resp.text()

    log("Parsing CSV...")
    reader = csv.DictReader(io.StringIO(text))
    public_ids = []
    total = 0
    for row in reader:
        total += 1
        if row.get("Is Public Domain", "").strip().lower() == "true":
            try:
                public_ids.append(int(row["Object ID"]))
            except (ValueError, KeyError):
                pass

    log(f"CSV: {total} total objects, {len(public_ids)} public domain")
    return public_ids


async def get_public_domain_ids_from_api(session):
    """Fallback: use search API pre-filter if CSV download fails."""
    url = f"{MET_API}/search?hasImages=true&isPublicDomain=true&q=*"
    log("Fetching pre-filtered IDs from search API...")
    async with session.get(url) as resp:
        resp.raise_for_status()
        data = await resp.json()
        ids = data["objectIDs"]
        log(f"Search API: {len(ids)} candidate IDs (note: ~28% are false positives)")
        return ids


async def fetch_object(session, object_id, proxy=None):
    """Fetch a single object. Returns (object_id, json_str|None, is_public, has_image, status)."""
    url = f"{MET_API}/objects/{object_id}"
    try:
        async with session.get(url, proxy=proxy) as resp:
            if resp.status == 200:
                data = await resp.json()
                is_public = data.get("isPublicDomain", False)
                has_image = bool(data.get("primaryImageSmall"))
                return object_id, json.dumps(data), is_public, has_image, "ok"
            elif resp.status == 404:
                return object_id, None, False, False, "404"
            elif resp.status in (429, 403):
                return object_id, None, None, None, "throttled"
            else:
                return object_id, None, None, None, f"http:{resp.status}"
    except Exception as e:
        return object_id, None, None, None, f"exc:{e}"


async def crawl_with_proxy(session, missing_ids, conn, proxy_url):
    """High-concurrency crawl using rotating proxy. Each request gets a different IP."""
    sem = asyncio.Semaphore(PROXY_CONCURRENCY)
    stats = {"valid": 0, "invalid": 0, "not_found": 0, "throttled": 0, "errors": 0}
    pending_inserts = []
    lock = asyncio.Lock()
    error_samples = []
    completed = 0
    total = len(missing_ids)
    start_time = time.time()
    last_log = start_time

    async def fetch_with_sem(oid):
        nonlocal completed, last_log
        async with sem:
            for attempt in range(MAX_RETRIES + 1):
                result = await fetch_object(session, oid, proxy=proxy_url)
                object_id, data_json, is_public, has_image, status = result
                if status != "throttled" or attempt == MAX_RETRIES:
                    break
                stats["throttled"] += 1
                await asyncio.sleep(2 ** attempt)  # exponential backoff

            async with lock:
                if status == "ok":
                    if is_public and has_image:
                        stats["valid"] += 1
                    else:
                        stats["invalid"] += 1
                    pending_inserts.append((object_id, data_json, is_public, has_image))
                    completed += 1
                elif status == "404":
                    stats["not_found"] += 1
                    pending_inserts.append((object_id, None, False, False))
                    completed += 1
                elif status == "throttled":
                    stats["errors"] += 1
                    completed += 1
                else:
                    stats["errors"] += 1
                    if len(error_samples) < 10:
                        error_samples.append(f"{object_id}:{status}")
                    if object_id is not None:
                        pending_inserts.append((object_id, None, None, None))
                    completed += 1

                if len(pending_inserts) >= COMMIT_EVERY:
                    conn.executemany(
                        "INSERT OR IGNORE INTO raw_artworks (met_object_id, data, is_public_domain, has_image) VALUES (?, ?, ?, ?)",
                        pending_inserts,
                    )
                    conn.commit()
                    pending_inserts.clear()

                now = time.time()
                if now - last_log >= 5:
                    last_log = now
                    elapsed = now - start_time
                    rate = completed / elapsed if elapsed > 0 else 0
                    eta_m = (total - completed) / rate / 60 if rate > 0 else 0
                    log(
                        f"  [{completed}/{total}] "
                        f"{rate:.1f} req/s | "
                        f"ETA {eta_m:.1f}m | "
                        f"valid={stats['valid']} invalid={stats['invalid']} "
                        f"404={stats['not_found']} throttled={stats['throttled']} err={stats['errors']}"
                    )

    log(f"Proxy mode: {PROXY_CONCURRENCY} concurrent requests via rotating proxy")
    await asyncio.gather(*[fetch_with_sem(oid) for oid in missing_ids])

    # Final commit
    if pending_inserts:
        conn.executemany(
            "INSERT OR IGNORE INTO raw_artworks (met_object_id, data, is_public_domain, has_image) VALUES (?, ?, ?, ?)",
            pending_inserts,
        )
        conn.commit()

    return stats, error_samples, completed, time.time() - start_time


async def crawl_direct(session, missing_ids, conn):
    """Burst+cooldown crawl without proxy (original approach)."""
    stats = {"valid": 0, "invalid": 0, "not_found": 0, "throttled": 0, "errors": 0}
    pending_inserts = []
    error_samples = []
    completed = 0
    start_time = time.time()
    total = len(missing_ids)
    retry_counts = {}
    retry_ids = []
    i = 0

    while i < total or retry_ids:
        burst = []
        while retry_ids and len(burst) < BURST_SIZE:
            burst.append(retry_ids.pop(0))
        while i < total and len(burst) < BURST_SIZE:
            burst.append(missing_ids[i])
            i += 1

        if not burst:
            break

        results = await asyncio.gather(*[fetch_object(session, oid) for oid in burst])

        batch_throttled = 0
        for object_id, data_json, is_public, has_image, status in results:
            if status == "ok":
                if is_public and has_image:
                    stats["valid"] += 1
                else:
                    stats["invalid"] += 1
                pending_inserts.append((object_id, data_json, is_public, has_image))
                completed += 1
                retry_counts.pop(object_id, None)
            elif status == "404":
                stats["not_found"] += 1
                pending_inserts.append((object_id, None, False, False))
                completed += 1
            elif status == "throttled":
                stats["throttled"] += 1
                batch_throttled += 1
                count = retry_counts.get(object_id, 0) + 1
                if count <= MAX_RETRIES:
                    retry_counts[object_id] = count
                    retry_ids.append(object_id)
                else:
                    stats["errors"] += 1
                    completed += 1
                    retry_counts.pop(object_id, None)
            else:
                stats["errors"] += 1
                if len(error_samples) < 5:
                    error_samples.append(status)
                completed += 1

        if len(pending_inserts) >= COMMIT_EVERY:
            conn.executemany(
                "INSERT OR IGNORE INTO raw_artworks (met_object_id, data, is_public_domain, has_image) VALUES (?, ?, ?, ?)",
                pending_inserts,
            )
            conn.commit()
            pending_inserts = []

        elapsed = time.time() - start_time
        rate = completed / elapsed if elapsed > 0 else 0
        remaining = total - completed
        eta_s = remaining / rate if rate > 0 else 0
        eta_h = eta_s / 3600
        log(
            f"  [{completed}/{total}] "
            f"{rate:.1f} req/s | "
            f"ETA {eta_h:.1f}h | "
            f"valid={stats['valid']} invalid={stats['invalid']} "
            f"404={stats['not_found']} throttled={stats['throttled']} err={stats['errors']}"
            + (f" retry_queue={len(retry_ids)}" if retry_ids else "")
        )

        if batch_throttled > 0:
            cooldown = BURST_COOLDOWN * 2
            log(f"  Throttled ({batch_throttled}/{len(burst)}), cooling down {cooldown}s...")
            await asyncio.sleep(cooldown)
        else:
            await asyncio.sleep(BURST_COOLDOWN)

    if pending_inserts:
        conn.executemany(
            "INSERT OR IGNORE INTO raw_artworks (met_object_id, data, is_public_domain, has_image) VALUES (?, ?, ?, ?)",
            pending_inserts,
        )
        conn.commit()

    return stats, error_samples, completed, time.time() - start_time


async def crawl(db_path=DB_PATH, limit=None, use_csv=True, proxy_url=None):
    conn = init_db(db_path)
    existing_ids = get_existing_ids(conn)
    log(f"Already in DB: {len(existing_ids)}")

    async with aiohttp.ClientSession() as session:
        if use_csv:
            try:
                candidate_ids = await get_public_domain_ids_from_csv(session)
            except Exception as e:
                log(f"CSV download failed ({e}), falling back to search API")
                candidate_ids = await get_public_domain_ids_from_api(session)
        else:
            candidate_ids = await get_public_domain_ids_from_api(session)

        all_missing = [oid for oid in candidate_ids if oid not in existing_ids]
        missing_ids = all_missing[:limit] if limit else all_missing
        log(f"Missing: {len(all_missing)} | To fetch this run: {len(missing_ids)}")

        if not missing_ids:
            log("Nothing to fetch.")
            conn.close()
            return

        if proxy_url:
            stats, error_samples, completed, elapsed = await crawl_with_proxy(
                session, missing_ids, conn, proxy_url
            )
        else:
            stats, error_samples, completed, elapsed = await crawl_direct(
                session, missing_ids, conn
            )

    log(f"\nDone in {elapsed/3600:.1f} hours ({elapsed/60:.0f} minutes)")
    log(f"  Fetched: {completed}")
    log(f"  Valid (public domain + image): {stats['valid']}")
    log(f"  Invalid (false positives): {stats['invalid']}")
    log(f"  Not found (404): {stats['not_found']}")
    log(f"  Throttled (retried): {stats['throttled']}")
    log(f"  Errors: {stats['errors']}")
    if error_samples:
        log(f"  Sample errors: {error_samples}")

    total_in_db = conn.execute("SELECT COUNT(*) FROM raw_artworks").fetchone()[0]
    valid_in_db = conn.execute(
        "SELECT COUNT(*) FROM raw_artworks WHERE is_public_domain = 1 AND has_image = 1"
    ).fetchone()[0]
    log(f"\nDB totals: {total_in_db} rows, {valid_in_db} valid")
    conn.close()


def main():
    parser = argparse.ArgumentParser(description="Crawl Met Museum API")
    parser.add_argument("--limit", type=int, help="Max objects to fetch (for testing)")
    parser.add_argument("--db", default=DB_PATH, help="Path to output database")
    parser.add_argument("--no-csv", action="store_true", help="Skip CSV download, use search API pre-filter")
    parser.add_argument("--proxy", type=str, help="Rotating proxy URL (e.g. http://user:pass@gw.dataimpulse.com:823)")
    args = parser.parse_args()
    asyncio.run(crawl(db_path=args.db, limit=args.limit, use_csv=not args.no_csv, proxy_url=args.proxy))


if __name__ == "__main__":
    main()
